...
if( ! empty( $_POST['email'] ) ) {
	...
	$receiver_email = 'info@helpnow.fr';

	// Email Receiver Name for SMTP Email
	$receiver_name 	= 'Info Help Now';

	// Email Subject
	$subject 	= 'Info Help Now';
	...
	if( $enable_mailchimp == 'no' ) { // Simple / SMTP Email
		...
		<tr>
		<td colspan="2" align="center" valign="top"><img style=" margin-top: 15px; " src="http://www.krskn.eu/images/logo-email-blue.png" ></td>
		</tr>
		<tr>
		...
		if( $enable_smtp == 'no' ) { // Simple Email
			...
			if( mail( $receiver_email, $subject, $message, $headers ) ) {
				...
				if( ! empty( $redirect_page_url ) ) {
			   	...
			   	//Success Message
			  	echo '{ "alert": "alert-success", "message": "Merci. Nous vous contacterons très rapidement !" }';
			} else {
				//Fail Message
			  	echo '{ "alert": "alert-danger", "message": "Aïe aïe. Nous rencontrons un problème." }';
			}
	...
	//Empty Email Message
	echo '{ "alert": "alert-danger", "message": "Veuillez mettre votre adresse e-mail !" }';
}